...
def get_shape(compute_client, compartment_id, availability_domain, name):
    ...
    vm_shapes = list(
        filter(lambda shape: shape.shape.startswith(name), shapes))
    if len(vm_shapes) == 0:
...
def add_route_rule_to_default_route_table_for_internet_gateway(
    ...
    get_route_table_response = virtual_network_client.get_route_table(
...
def clear_route_rules_from_default_route_table(virtual_network_composite_operations, vcn):
    ...
    print('Cleared Route Rules from Route Table: {}'.format(
        vcn.default_route_table_id))
    print()
...
def remove_network_security_group_security_rules(virtual_network_client, network_security_group):
    ...
    print('Removed all Security Rules in Network Security Group: {}'.format(
        network_security_group.id))
    print()
...
def get_launch_instance_details(compartment_id, availability_domain, shape, image, subnet, ssh_public_key):
    ...
    result_name = str(shape.shape) + "-" + str(OCPU) + "-" + str(RAM)
    ...
    launch_instance_details = oci.core.models.LaunchInstanceDetails(
        ...
        agent_config=oci.core.models.LaunchInstanceAgentConfigDetails(
            ...
            plugins_config=[oci.core.models.InstanceAgentPluginConfigDetails(
                name='Vulnerability Scanning', desired_state='DISABLED'), oci.core.models.InstanceAgentPluginConfigDetails(name='Compute Instance Monitoring', desired_state='DISABLED'), oci.core.models.InstanceAgentPluginConfigDetails(name='Bastion', desired_state='DISABLED')]
        )
    )
...
def launch_instance_and_wait_for_work_request(compute_client_composite_operations, launch_instance_details):
    ...
    get_instance_response = compute_client_composite_operations.client.get_instance(
        instance_id)
...
def print_instance_details(compute_client, virtual_network_client, instance):
    ...
    get_vnic_response = virtual_network_client.get_vnic(
...
def print_work_request_details(work_request_client, work_request_id):
    get_work_request_response = work_request_client.get_work_request(
        work_request_id)
    work_request_details = get_work_request_response.data

    list_errors_response = work_request_client.list_work_request_errors(
        work_request_id)
    ...
    log_limit = 20
    page_size = 5
    resp = oci.pagination.list_call_get_up_to_limit(
        work_request_client.list_work_request_logs, log_limit, page_size, work_request_id)
...
if __name__ == "__main__":
    OCPU, RAM, SHAPE = int(sys.argv[1]), int(sys.argv[2]), sys.argv[3]
    ...
    virtual_network_composite_operations = oci.core.VirtualNetworkClientCompositeOperations(
        virtual_network_client, signer=signer)
    work_request_client = oci.work_requests.WorkRequestClient(
        config, signer=signer)
    object_storage_client = oci.object_storage.ObjectStorageClient(
        config, signer=signer)

    availability_domain = get_availability_domain()
    shape = get_shape(compute_client, compartment_id,
                      availability_domain, SHAPE)
    ...
    try:
        vcn = create_vcn(virtual_network_composite_operations,
                         compartment_id, cidr_block)
        subnet = create_subnet(
            virtual_network_composite_operations, vcn, availability_domain)
        internet_gateway = create_internet_gateway(
            virtual_network_composite_operations, vcn)
        add_route_rule_to_default_route_table_for_internet_gateway(
            virtual_network_client, virtual_network_composite_operations, vcn, internet_gateway
        )
        network_security_group = create_network_security_group(
            virtual_network_composite_operations, compartment_id, vcn)
        add_network_security_group_security_rules(
            virtual_network_client, network_security_group)
        launch_instance_details = get_launch_instance_details(
            compartment_id, availability_domain, shape, image, subnet, ssh_public_key
        )
        launch_instance_details.create_vnic_details.nsg_ids = [
            network_security_group.id]
        ...
        print_instance_details(
            compute_client, virtual_network_client, instance_with_network_security_group)

        while True:
            user_input = input("Do you want to clean all? (yes/no): ")
        ...
        if instance_with_network_security_group:
            terminate_instance(compute_client_composite_operations,
                               instance_with_network_security_group)
        if instance_via_work_requests:
            terminate_instance(
                compute_client_composite_operations, instance_via_work_requests)
        if instance:
        ...
        if network_security_group:
            ...
            delete_network_security_group(
                virtual_network_composite_operations, network_security_group)
        ...
        if internet_gateway:
            ...
            clear_route_rules_from_default_route_table(
                virtual_network_composite_operations, vcn)
            delete_internet_gateway(
                virtual_network_composite_operations, internet_gateway)
